      ...
      return (
        <div>
                ...
                <div className="popup-contenedor">
                    ...
                    <div className="profile">
                      ...
                      <div className="profession-list">
                        ...
                        {item.professions.map(item=> <div key={item}>{item}</div>)}
                      </div>
                      <hr/>                    
                      <div className="friend-list">
                        <label>Friend List:</label>
                        {item.friends.length===0 && <div style={{color:'red'}}>Without friends</div>}
                        {item.friends.map(item=> <div key={item}>{item}</div>)}
                      </div>                      
                    </div>
          ...
          <div className="gnome-detail" >
            <img src={item.thumbnail} alt={item.name} />
            <label className="gnome-name">{item.name.substring(0, 18)}</label><br/>
            <label className="gnome-extra-info">Age: {item.age} - Gender :{item.hair_color==='Pink'?'F':'M'}</label>
